def mitigation_down(request, route):
    ...
    if ideal_distance == covered_distance:
        ...
        if m_speed > 90:
            ...
            report = "There is a major delay in the route. The delay cannot be mitigated completely. The train must maintain an average speed above " + str(
                m_speed) + " kmph to minimize the delay."
            ...
            res = ("There is a minor delay in the route.\n"
                   "For the delay to be mitigated the train must maintain an average speed of {} kmph for the remaining distance.".format(
                round(m_speed, 2)))
            report = "There is a minor delay in the route. For the delay to be mitigated the train must maintain an average speed of" + str(
                round(m_speed, 2)) + "kmph"
            with open('statics/csv/delay.csv', 'a', newline='') as file:
                writer = csv.writer(file)
